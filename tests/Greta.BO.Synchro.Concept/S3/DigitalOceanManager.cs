namespace Greta.BO.Synchro.Concept.S3
{
    ...
    public class DigitalOceanManager
    {
        ...
        public DigitalOceanManager(string accessKey, string secretKey, string spaceName, string serviceUrl = "https://nyc3.digitaloceanspaces.com")
        ...
        /// <param name="spaceName">Space name.</param>
        /// <param name="serviceUrl">Service URL.</param>
        public DigitalOceanManager(KeyManager keyManager, string spaceName, string serviceUrl = "https://nyc3.digitaloceanspaces.com")
            : this(spaceName, serviceUrl)
        {
            if (keyManager == null)
        ...
        private DigitalOceanManager(string spaceName, string serviceUrl = "https://nyc3.digitaloceanspaces.com")
        {
        ...
        /// <summary>
        /// Check if this space Name exists
        /// </summary>
        /// <returns></returns>
        public async Task<bool> SpaceExist()
        {
            using (var client = CreateNewClient())
            {
                var currentBucket = await client.ListBucketsAsync();
                foreach (var bucket in currentBucket.Buckets)
                {
                    if (bucket.BucketName == _spaceName)
                        return true;
                }
            }
            return false;
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<bool> CreateSpace()
        {
            using (var client = CreateNewClient())
            {
                var currentBucket = await client.PutBucketAsync(_spaceName);
                return currentBucket.HttpStatusCode == System.Net.HttpStatusCode.OK;
            }
        }
