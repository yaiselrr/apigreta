...
namespace Greta.BO.BusinessLogic.Handlers.Queries.AdBatch
{
    ...
    /// </summary>
    public static class AdBatchFilter
    {
        public record Query(int CurrentPage, int PageSize, AdBatchSearchModel Filter) : IRequest<Response>, IAuthorizable
        {
        ...
        public class Handler : IRequestHandler<Query, Response>
        {
            ...
            public async Task<Response> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var entities = await _service.Filter(
                    request.CurrentPage,
                    request.PageSize,
                    _mapper.Map<Api.Entities.AdBatch>(request.Filter),
                    request.Filter.Search,
                    request.Filter.Sort);
                return new Response {Data = _mapper.Map<Pager<AdBatchModel>>(entities)};
            }
        }

        public record Response : CQRSResponse<Pager<AdBatchModel>>;
    }
}