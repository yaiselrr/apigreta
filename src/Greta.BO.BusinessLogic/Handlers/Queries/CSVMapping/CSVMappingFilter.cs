...
{
    public static class CSVMappingFilter
    {
        public record Query
            (int CurrentPage, int PageSize, CSVMappingSearchModel Filter, ModelImport modelImport) : IRequest<Response>//, IAuthorizable
        {
        ...
        public class Handler : IRequestHandler<Query, Response>
        {
            ...
            public async Task<Response> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var entities = await _service.Filter(
                    request.CurrentPage,
                    request.PageSize,
                    _mapper.Map<Api.Entities.CSVMapping>(request.Filter),
                    request.Filter.Search,
                    request.Filter.Sort,
                    request.modelImport);
                return new Response {Data = _mapper.Map<Pager<CSVMappingModel>>(entities)};
            }
        }

        public record Response : CQRSResponse<Pager<CSVMappingModel>>;
    }
}