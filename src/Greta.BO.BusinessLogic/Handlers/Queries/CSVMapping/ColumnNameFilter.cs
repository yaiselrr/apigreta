...
{
    public static class ColumnNameFilter
    {
        public record Query(int CurrentPage, int PageSize, ColumnNameSearchModel Filter) : IRequest<Response>//, IAuthorizable
        {
        ...
        public class Handler : IRequestHandler<Query, Response>
        {
            ...
            public async Task<Response> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var entities = await _service.FilterColumnName(
                    request.CurrentPage,
                    request.PageSize,
                    _mapper.Map<ColumnNameModel>(request.Filter),
                    request.Filter.Search,
                    request.Filter.Sort,request.Filter.modelImport);
                return new Response {Data = _mapper.Map<Pager<ColumnNameModel>>(entities)};
            }
        }

        public record Response : CQRSResponse<Pager<ColumnNameModel>>;
    }
}