...
{
    public static class RancherFilter
    {
        public record Query(int CurrentPage, int PageSize, RancherSearchModel Filter) : IRequest<Response>, IAuthorizable
        {
        ...
        public class Handler : IRequestHandler<Query, Response>
        {
            ...
            public async Task<Response> Handle(Query request, CancellationToken cancellationToken)
            {
                ...
                var entities = await _service.Filter(
                    request.CurrentPage,
                    request.PageSize,
                    _mapper.Map<Api.Entities.Rancher>(request.Filter),
                    request.Filter.Search,
                    request.Filter.Sort);
                return new Response {Data = _mapper.Map<Pager<RancherModel>>(entities)};
            }
        }

        public record Response : CQRSResponse<Pager<RancherModel>>;
    }
}